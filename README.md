...
安装时只需要下载并运行 `Mineradio-1.1.1-Setup.exe`，Old-Legacy 卸载程序若无安全标记会被跳过，安装程序仅会删除旧的卸载文件与注册表项，防止触发旧版不安全的卸载流程。不要下载 `Source code`、`.blockmap`、`latest.yml`，也不要把 `win-unpacked` 当成正式安装包。
...
- QQ 音乐搜索、登录态与音源补充接入
- 接入自建 Navidrome 私有云音乐服务器，支持 3D 封面前端跨域代理及浏览器不兼容高保真无损格式(ALAC/APE等)实时无感转码
- 针对 Linux (AppImage) 及 Wayland 环境优化窗口 WM_CLASS 映射和跨平台桌面图标关联
